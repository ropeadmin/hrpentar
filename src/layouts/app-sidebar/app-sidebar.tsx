...
import {
  adminSidebarfooterItems,
  adminSidebarItems,
  employeeSidebarItems,
} from '@/utils/sidebar-path/sidebar.path';
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
              ...
              <Collapsible key={index} className="group/collapsible ">
                <SidebarMenuItem>
                  <CollapsibleTrigger asChild className="">
                    <SidebarMenuButton
                      asChild
                      className="pointer-events-autohover:bg-n800 data-[state=open]:hover:bg-n800 hover:bg-n800 active:bg-n800"
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {item.pending && (
                          <div className="bg-n600 text-white text-[10px] font-medium rounded-sm px-1 py-px">
                            Coming Soon
                          </div>
                        )}
                      </div>
                  ...
                  <CollapsibleContent>
                    {item.subItems &&
                      item.subItems.map((subItem, index) => (
                        <SidebarMenuSub
                          key={index}
                          className="py-2 hover:bg-[#182434] border-none rounded-[5px] cursor-pointer"
                        >
                          <Link href={subItem.url}>
                            <SidebarMenuSubItem className="text-white text-sm font-normal pl-4 select-none">
                              {subItem.name}
                            </SidebarMenuSubItem>
                          </Link>
                        </SidebarMenuSub>
            ...
            <SidebarMenuItem key={item.title}>
              ...
              >
                <Link href={item.url} className="flex items-center gap-[10px]">
                  ...
                  <span className="w-full font-[400] text-sm text-white">
                    {item.title}
                  </span>
                </Link>
              </SidebarMenuButton>
